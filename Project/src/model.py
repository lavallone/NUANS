...
class MatchSum(pl.LightningModule):
	...
	def forward(self, x):
		...
		del text
		chunk_text_embed = chunk_text_embed.detach().cpu()
		...
		for i in range(candidates_num):
			...
			chunk_cand_embed = chunk_cand_embed.detach().cpu()
		...
		for i in range(3):
			...
			chunk_gold_embed = chunk_gold_embed.detach().cpu()
		...
		for i in range(t.shape[0]): # i.e. self.hparams.batch_size  
			...
			gold_score_list.append(torch.cosine_similarity(gold_embed_stack, new_text_embed, dim=-1).mean())
		...
		del abstractive
		chunk_abstr_embed = chunk_abstr_embed.detach().cpu()
		...
		# I need to enable the gradient computations!!!
	...
	def prediction(self, ids, candidates_scores):
		best_candidate_indeces = torch.argmax(candidates_scores, dim=1) # (1, batch)
		ris = []
		for story, idx in list(zip(ids, best_candidate_indeces)):
	...
	def compute_ROUGE(self, ids, best_candidates):
		...
		for story, best_candidate in list(zip(ids, best_candidates)):
			gold_list = FairySum_DataModule.gold[story]
	...
	def validation_step(self, batch, batch_idx):
		cand_score, gold_score, abstr_score = self(batch)
		# LOSS
		self.log("val_loss", self.loss_function(cand_score, gold_score, abstr_score)["loss"], on_step=False, on_epoch=True, batch_size=self.hparams.batch_size)
		# F1 Score ROUGE-L
		pred = self.prediction(batch["id"], cand_score) # it returns the list of the best summaries for each story
