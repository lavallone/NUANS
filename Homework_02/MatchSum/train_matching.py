...
def test_model(args):
    ...
    # load dataset
    data_paths = get_data_path(args.mode, args.encoder) # è il test_CNNDM_roberta.jsonl file da dove prendiamo i dati
    datasets = MatchSumPipe(args.candidate_num, args.encoder).process_from_file(data_paths) # it returns a DataBundle class from fastNLP library
    print('Information of dataset is:')
    print(datasets)
    ...
    for cur_model in models: # models is a list of all the pretrained models saved in the "save_path" folder
        
        print('Current model is {}'.format(cur_model))

        # load model
        model = torch.load(join(args.save_path, cur_model)) # we load the pretrained model!
        ...
        # we use a class implemented by the fastNLP library
        tester = Tester(data=test_set, model=model, metrics=[test_metric], 
                        batch_size=args.batch_size, device=device, use_tqdm=True) #use_tqdm=False
        tester.test()
...
if __name__ == '__main__':
    ...
    if args.mode == 'train':
        ...
        train_model(args)
    else: # we'll never train this model, we just want to test in on our dataset!
